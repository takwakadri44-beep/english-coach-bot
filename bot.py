...
@app.route('/')
def home():
    return "Your Bot is Alive and Running!"

def run_flask():
    port = int(os.environ.get("PORT", 8080))
    app.run(host='0.0.0.0', port=port)

# 2. جلب المفاتيح البيئية بأمان
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN")
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY")

# 3. تهيئة إعدادات جيمني والبوت بالطريقة القياسية الصحيحة
genai.configure(api_key=GEMINI_API_KEY)
bot = telebot.TeleBot(TELEGRAM_TOKEN)

# استدعاء الموديل القياسي المدعوم عالمياً
...
def handle_msg(message):
    ...
    if state["waiting_for_correction"]:
        check_prompt = f"The user had to correct their mistake. Original text had an error. They just wrote: '{user_text}'. Is it grammatically 100% correct now? Answer ONLY with 'YES' or 'NO'."
        try:
            check_res = model.generate_content(check_prompt).text.strip().upper()
            if "YES" in check_res:
    ...
    try:
        ...
        bot.reply_to(message, "⚠️ عذراً، هناك مشكلة مؤقتة في معالجة الرد الذكي. جرب إرسال الرسالة مرة أخرى.")
        print(f"Error: {e}")
